            ...
            <div className="card p-8 text-center mb-6">
              ...
              <p className="text-gray-400 mb-6">
                ...
                <span className="text-white font-bold">
                  {String(topic._count.cards).padStart(2, '0')}
                </span>{' '}
            ...
            {timerOn && gameState === 'playing' && (
              <div className="mb-4">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className={`text-sm font-medium tabular-nums ${timeLeft <= 5 ? 'text-red-400' : 'text-gray-400'}`}>
                    {timeLeft}s
                  </span>
                </div>
              ...
              {gameState === 'playing' && (
                <>
                  {currentCard.cardType === 'IDENTIFICATION' ? (
                    ...
                    /* Uses stable shuffledChoices from useState — never reshuffles on timer tick */
                    <div className="space-y-2">
                      {shuffledChoices.map((choice, i) => (
                        <button
                          key={i}
                          onClick={() => submitAnswer(choice)}
                          className="w-full text-left px-4 py-3 bg-gray-800 hover:bg-gray-700 border border-gray-700 hover:border-indigo-500/50 rounded-xl text-white text-sm transition-all active:scale-[0.98]"
                        >
                          <span className="text-gray-500 mr-3 font-medium">
